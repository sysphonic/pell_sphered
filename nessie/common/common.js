...
function replaceClassName(elem, orgClassName, className)
{
  if (!elem || !orgClassName) {
    return;
  }
  if (!className) {
    removeClassName(elem, orgClassName);
    return;
  }
  var classNames = elem.className.split(/\s/);
  var idx = classNames.indexOf(orgClassName);
  if (idx >= 0) {
...
function getElemByTagNameInParentNodes(elem, tagName, includeSelf)
{
  if (!elem || !tagName) {
    return null;
  }
  ...
  var node = (includeSelf)?(elem):(elem.parentNode);
  for (var i=0; node; node=node.parentNode) {
    if (node.tagName
...
function getFormValue(name, visibleOnly)
{
  var elem = getFormElem(name, visibleOnly);
  if (elem) {
    return elem.value;
  } else {
    return null;
...
function getFormElem(name, visibleOnly)
{
  var ret = null;
  ...
  if (entries) {
    for (var i=0; i < entries.length; i++) {
      ...
      if (visibleOnly
          && (entry.type == "hidden" || !isElemDisplayed(entry))) {
        continue;
      }
      ret = entry;
    }
...
{
  options = (options || "directories=no, location=no, resizable=no");
...
function setListText(list, val, text)
{
  if (!list) {
    return null;
  }
...
function getUtcExp(date)
{
  var exp = "";
  date = (date || new Date());

  exp += date.getUTCFullYear();
  exp += "-";
  exp += fillLeft(String(date.getUTCMonth()+1), "0", 2);
  exp += "-";
  exp += fillLeft(String(date.getUTCDate()), "0", 2);
  exp += " ";
  exp += fillLeft(String(date.getUTCHours()), "0", 2);
  exp += ":";
  exp += fillLeft(String(date.getUTCMinutes()), "0", 2);
  exp += ":";
  exp += fillLeft(String(date.getUTCSeconds()), "0", 2);
  exp += ".";
  exp += String((date.getUTCMilliseconds() / 1000).toFixed(3).slice(2, 5));
...
function getStyleString(elem)
{
  var ret = "";
  for (var item in elem.currentStyle) {
    var itemcv = item.replace(/[A-Z]/g, function(whole, p1){ return ("-" + item.charAt(p1).toLowerCase())});
    ret += itemcv + ":" + elem.currentStyle.getAttribute(item) + ";";
  }
...
function selectAll(classNames)
{
  ...
  for (var i=0; i < classNames.length; i++) {
    ...
    elems = elems.concat(collectionToArray(document.getElementsByClassName(className)));
  }

  var allSelected = true;
  for (var i=0; i < elems.length; i++) {
    if (!isElemDisplayed(elems[i]) || elems[i].disabled) {
      continue;
    }
    if (!elems[i].checked) {
      allSelected = false;
...
function getParentIframe()
{
  ...
  return parent.document.getElementById(window.name);
}

function getOwnerWindow(elem)
{
